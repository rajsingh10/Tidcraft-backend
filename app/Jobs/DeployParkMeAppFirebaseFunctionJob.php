...
class DeployParkMeAppFirebaseFunctionJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $enabled = config('services.parkmeapp.enable_cloudfunction_deploy', env('ENABLE_CLOUDFUNCTION_DEPLOY', false));
        if (!$enabled) {
        ...
        try {
            ...
            if ($exitCode === 0) {
                ...
                Log::error("ParkMeApp Cloud Function deploy returned exit code {$exitCode} for database {$this->databaseId}");
            }
        } catch (\Throwable $e) {
